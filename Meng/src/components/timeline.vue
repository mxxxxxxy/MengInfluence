...
export default{
    ...
    methods:{
        drawTimeline() {
            ...
            // 创建一个颜色插值函数
            const colorInterpolator = d3.interpolate("#D8D9D5", "#5E5D5B");  // 从浅灰色到深灰色
            // 创建一个颜色比例尺
                ...
                let image = svg.append("image")  // 在循环内部为 image 变量赋值
                    ...
                    .on("mouseover", (event, d) => {
                        ...
                        svg.append("text")
                            ...
                            .attr("writing-mode", "tb")  // 设置文字从上到下排列
                            .text(book.data.name);
                        // d3.select("#rect-" + index).attr("opacity", 0);
                    })
                    .on("mouseout", (event, d) => {
                        // 恢复当前的图片
