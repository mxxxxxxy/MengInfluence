...
export default{
    data() {
        return {
            books: book_data.map(book => {
                console.log(tree(book).data.writing_year);  // 打印单个 book 对象
                return tree(book);
            })
        }
    },
    ...
    for (let i = 0; i < 3; i++) {
        ...
        options.forEach((option, j) => {
            // 添加正方形
            svg.append("rect")
                .attr("x", width * 0.2  + i * 100)  // 按钮的 x 坐标，设置为一个固定的值
            ...
            svg.append("text")
                .attr("x", width * 0.2 + width * 0.15 + i * 100)  // 文本的 x 坐标，设置为一个固定的值
